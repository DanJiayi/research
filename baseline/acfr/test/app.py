...
def predict():
    ...
    create_synthetic_data(dataset="news", dir="data_utils", run_n=0, selection_bias=2)

    checkpoint_path = os.getenv('MODEL_PATH', f"models/checkpoint.pth")

    data_path = os.getenv('DATA_PATH', f"data_utils/0/test.csv")
    
    label_path = os.getenv('LABEL_PATH', f"data_utils/0/out_t_grids.csv")
