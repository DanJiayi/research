...
if __name__ == "__main__":
    ...
    for model_name in ['CVRNet_tr']:  #,'CVRNet'
        ...
        best_params = None
        for h in [8,16,32,64]:
            for init_lr in [1e-6,1e-5,5e-5,1e-4,1e-3]: #1e-6,
                for tr_init_lr in [1e-5,5e-5,1e-4,5e-4,1e-3]:
                    if h==8 and (init_lr<5e-5 or (init_lr==5e-5 and tr_init_lr<=1e-4)): continue
                    flag = 0
                    ...
                    for epoch in range(n):
                        ...
                        if epoch % 5 == 0:
                            ...
                            if auuc2>0 and qini2>0:
                                if (1.5*auuc2+qini2) > (1.5*best_auuc+best_qini):
                                    best_auuc,best_qini = auuc2,qini2
                                    best_params = params

                                if auuc2>=0.03 and qini2>=0.05:
                                    ckpt_dir = os.path.join(cur_save_path, "checkpoints")
                                    os.makedirs(ckpt_dir, exist_ok=True)
                                    ckpt_name = f"{model_name}_ckpt_{params}_{'_'.join([f'{r:.4f}' for r in [auuc2,qini2]])}.pth.tar"
                            ...
                            with open(save_path + f'/result_tune2.json', 'w') as fp:
                                json.dump(Result, fp)
