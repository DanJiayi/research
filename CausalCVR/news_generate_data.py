...
if __name__ == "__main__":
    ...
    v4 = np.random.randn(num_feature)
    v4 = v4/np.sqrt(np.sum(v4**2))

    def x_t(x):
        alpha = 2
        tt = np.sum(v4 * x) / (2. * np.sum(v3 * x))
    ...
    def t_x_y(t, x):
        res1_y1 = max(-2, min(2, np.exp(0.3 * (np.sum(3.14159 * np.sum(v3 * x) / np.sum(v4 * x)) - 1))))
        res1_y2 = max(-1, min(1, np.exp(0.5 * (np.sum(3.14159 * np.sum(v3 * x) / np.sum(v4 * x)) - 1))))
        res2_y1 = 20. * (np.sum(v1 * x))
        res2_y2 = 20. * (np.sum(v2 * x))
        y1 = 2 * (4 * (t - 0.5)**2 * np.sin(0.5 * 3.14159 * t)) * (res1_y1 + res2_y1)
        y2 = 2 * (-2 * (t + 0.1)**2 * np.cos(0.5 * 3.14159 * t**2)) * (res1_y2 + res2_y2)
        return y1,y2
    ...
    def news_matrix():
        data_matrix = torch.zeros(num_data, num_feature+4)
        ...
        for _ in range(num_data):
            ...
            data_matrix[_, num_feature+2] = y2
            data_matrix[_, num_feature+3] = y1 * y2
            data_matrix[_, 1: num_feature+1] = x

        # get t_grid
        t_grid = torch.zeros(3, num_data)
