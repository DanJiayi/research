...
for h in [8,16,32]:
    for init_lr_nuis in [5e-5,1e-4,5e-4]:
        for final_lr in [5e-3]:
            params = f"h{h}_init{init_lr_nuis}_final{final_lr}"
            Result[params] = []
            ...
            for ds_idx in range(num_dataset):
                ...
                print(f"\n===== [{params}] Dataset {ds_idx} | Training Nuisance Model1 on A =====")
                model1 = train_nuisance(model1, loader_A, num_epoch, init_lr_nuis, verbose)
                print(f"===== [{params}] Dataset {ds_idx} | Training Nuisance Model2 on B =====")
                ...
                with torch.no_grad():
                    ...
                    out1_on_B = model1(tB, xB)
                    out2_on_A = model2(tA, xA)
                ...
                print(f"\n===== [{params}] Dataset {ds_idx} | Training Final Model1 =====")
                final_model1 = train_final(final_model1, t_all, X_all, PSI1,
                                           epochs=num_epoch, lr=final_lr)
                print(f"\n===== [{params}] Dataset {ds_idx} | Training Final Model2  =====")
                ...
                print(f"===> [{params}] ds={ds_idx}  mse1={mse1:.5f} mse2={mse2:.5f}")

                Result[params].append([mse1, mse2])
                with open(os.path.join(save_path, 'result_dr_tune2.json'), 'w') as fp:
                    json.dump(Result, fp)
