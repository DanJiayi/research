...
import math
import numpy as np
from models.dynamic_net import Vcnet_2, Drnet, TR
...
def criterion_TR_cvr(out, trg, y1, y2, beta=1., epsilon=1e-9):
    ...
    return beta * (( (y2 - out2)/(out1 + epsilon) - (y1-out1)*out2/(out1**2+epsilon) - trg/(out[0].squeeze()+epsilon) )**2).mean() #.detach()

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='train with ihdp data_utils')

    # i/o
    parser.add_argument('--data_dir', type=str, default='dataset/ihdp', help='dir of data_utils matrix')
    parser.add_argument('--data_split_dir', type=str, default='dataset/ihdp/eval', help='dir of data_utils split')
    parser.add_argument('--save_dir', type=str, default='logs/ihdp/tune', help='dir to save result')
    ...
    for num_epoch in [100,150,200,250,300]:
        for h in [8]:
            params = f'{num_epoch}_{h}'
            Result[params]={}
            #Result[model_name]=[]
            k+=1
